...
import ContactForm from 'components/ContactForm';
 ...
 class App extends Component {
  ...
  getFormData = data => {
    ...
    const dataIncludes = this.state.contacts.find(contact => contact.name === data.name);
    if(dataIncludes) {
  ...
  render() {
    return (
      <div>
        ...
        <h2>Contacts</h2>
        <Filter handleFilterChange={this.handleFilterChange} filter={this.state.filter}/>
        <ContactsList contacts={this.state.contacts} filter={this.state.filter} onClick={this.handleDelete}/>
      </div>
    );
  }
}
